...
"""
This script evaluates TACO compression with different caption scenarios on the Kodak dataset:
1. Correct caption (from OFA)
2. No caption
3. Random caption from another image

It processes all 24 Kodak images and computes the average LPIPS score for each scenario.
"""
...
def main():
    ...
    for image_file in tqdm(image_files, desc="Processing Kodak images"):
    ...
    results_data = {
        "samples": image_files,
        "individual_results": {
            "correct_caption": {image_files[i]: {"lpips": results["correct_caption"][i], "bpp": bpp_results["correct_caption"][i]} for i in range(len(image_files))},
            "no_caption": {image_files[i]: {"lpips": results["no_caption"][i], "bpp": bpp_results["no_caption"][i]} for i in range(len(image_files))},
            "random_caption": {image_files[i]: {"lpips": results["random_caption"][i], "bpp": bpp_results["random_caption"][i]} for i in range(len(image_files))}
        },
        "average_results": {
    ...
    with open("kodak_caption_impact_results.json", "w") as f:
        json.dump(results_data, f, indent=2)
    
    print("\nDetailed results saved to kodak_caption_impact_results.json")
